...
class ClassDeclaration extends Declaration {
  ...
  String fieldToCodeString() {
    return fields
        .map(
          (field) =>
              '${documentationCommentsToCodeString(field.documentationComments)}  final ${field.type.toCodeString()} ${field.name};\n\n',
        )
        .safeJoin();
  }
...
class StaticField {
  ...
  String toCodeString() {
    ...
    return '${documentationCommentsToCodeString(
      documentationComments,
    )}  static ${toCodeAndIsConst.isConst() ? 'const ' : ''}${type.toCodeString()} $name = ${toCodeAndIsConst.code};\n';
  }
}
...
class Method {
  ...
  String toCodeString() {
    return '${documentationCommentsToCodeString(documentationComments)}${methodType == MethodType.override ? '@override\n' : ''}${useResultAnnotation ? '@useResult\n' : ''}${methodType == MethodType.static ? 'static ' : ''}${returnType.toCodeString()} ${isGetter ? 'get ' : ''}${methodNameConsiderOperator()}${typeParameters.isEmpty ? '' : '<${stringListJoinWithComma(typeParameters)}>'}${isGetter ? '' : Parameter.toCodeString(false, parameters)}${statements.isEmpty ? ';\n' : '${isAsync ? ' async' : ''} {\n${statements.map(
...
class Parameter {
  ...
  String? _namedArgumentToCodeString(bool isConstructor) {
    return parameterPattern.match(
      ...
      namedWithDefault: (n) =>
          '${_nameWithThis(isConstructor)}${n.constDefaultExpr is ExprNull ? '' : '= ${n.constDefaultExpr.toCodeAndIsConst().toCodeString(true)}'},',
    );
  }
...
class TypeFunction implements Type {
  ...
  @override
  String toCodeString() {
    return '${returnType.toCodeString()} Function(${parameters.map((parameter) => '${parameter.second.toCodeString()} ${parameter.first}').safeJoin(',')})${isNullable ? '?' : ''}';
  }
...
class TypeNormal implements Type {
  ...
  String toCodeString() {
    return name +
        (arguments.isEmpty
            ? ''
            : '<${arguments.map((argument) => argument.toCodeString()).safeJoin(', ')}>') +
        (isNullable ? '?' : '');
  }
...
class EnumDeclaration extends Declaration {
  ...
  @override
  String toCodeString() {
    return '${documentationCommentsToCodeString(documentationComments)}enum $name${implementsClassList.isEmpty ? '' : ' implements ${implementsClassList.safeJoin(',')}'} {\n${enumValues.map((enumValue) => enumValue.toCodeString()).safeJoin('\n')}${methods.isEmpty ? '' : ';${methods.map((method) => method.toCodeString()).safeJoin(
          '\n',
        )}'}}';
  }
}
...
class StatementSwitch implements Statement {
  ...
  @override
  String toCodeString() {
    return 'switch (${expr.toCodeAndIsConst().toCodeString(true)}) {\n${patternList.map(
          (pattern) =>
              'case${pattern.first.toCodeAndIsConst().toCodeString(true)}: {\n${pattern.second.map(
                    (statement) => statement.toCodeString(),
                  ).safeJoin()}\n}\n',
        ).safeJoin('\n')}}\n';
  }
  ...
  CodeAndIsConst toCodeAndIsConst() {
    ...
    return CodeAndIsConst(
      isDoubleQuote
          ...
          : "'${items.map((item) => item is StringLiteralItemNormal ? item.toCodeAndIsConst().code.replaceAll("'", r"\'") : item.toCodeAndIsConst().code).safeJoin()}'",
  ...
  CodeAndIsConst toCodeAndIsConst() {
    ...
    return CodeAndIsConst(
      '{${stringListJoinWithComma(
        IList(codeAndIsConstIter.map(
          (item) =>
              '${item.first.toCodeString(!isAllConst)}: ${item.second.toCodeString(!isAllConst)}',
        )),
      )}}',
      isAllConst ? ConstType.explicit : ConstType.noConst,
    );
  ...
  return CodeAndIsConst(
    '(${stringListJoinWithComma(IList([
          ...
          ...namedArgumentsCodeAndIsConst.map(
            (argument) =>
                '${argument.first}: ${argument.second.toCodeString(!isAllConst)}',
          ),
        ]))})',
    isAllConst ? ConstType.explicit : ConstType.noConst,
  );
