...
class SimpleDartCode {
  ...
  String toCodeString() {
    final importPackageFileNameAndAsNameList = IList([
      const ImportPackageFileNameAndAsName(packageAndFileName: 'package:meta/meta.dart'),
      const ImportPackageFileNameAndAsName(packageAndFileName: 'package:fast_immutable_collections/fast_immutable_collections.dart'),
      ...importPackageAndFileNames,
    ]).sort((a, b) => comparePackageName(a.packageAndFileName, b.packageAndFileName)).divideIn2(
          (item) => item.packageAndFileName.startsWith('./'),
        );

    final importStatementAbsolute = importPackageFileNameAndAsNameList.last.map((n) => n.toImportStatementCodeString()).safeJoin('\n');
    final importStatementRelative = importPackageFileNameAndAsNameList.first.map((n) => n.toImportStatementCodeString()).safeJoin('\n');
    final declarationListCode = declarationList.map((declaration) => declaration.toCodeString()).safeJoin('\n\n');
...
class ImportPackageFileNameAndAsName {
  ...
  String toImportStatementCodeString() {
    ...
    return "import '" + packageAndFileName + "'" + (asNameStringOrNull == null ? '' : 'as ' + asNameStringOrNull) + ';';
  }
}
...
class ClassDeclaration extends Declaration {
  ...
  @override
  String toCodeString() {
    return documentationCommentsToCodeString(documentationComments) + '@immutable\n' + (isAbstract ? 'abstract ' : '') + 'class ' + name + ' ' + implementsClassListToCodeString() + '{\n' + constructorToCodeString() + fieldToCodeString() + staticFieldsToCodeString() + methodsToCodeString() + '\n}';
  }
  ...
  String fieldToCodeString() {
    return fields
        .map(
          (field) => documentationCommentsToCodeString(field.documentationComments) + '  final ${field.type.toCodeString()} ${field.name};\n\n',
        )
        .safeJoin();
  }

  String constructorToCodeString() {
    return documentationCommentsToCodeString(documentationComments) + '  const ' + name + (isPrivateConstructor ? '._' : '') + constructorFieldsToCodeString() + ';\n';
  }
  ...
  String methodsToCodeString() {
    return [
      ...
      if (!isAbstract) ...[hashCodeMethod(), equalMethod()],
      ...methods,
      ...
      statements: IList([
        StatementReturn(
          oneField == null
              ...
              : ExprGet(expr: ExprVariable(oneField.name), fieldName: 'hashCode'),
        )
      ]),
    );
  }
  ...
  Method equalMethod() {
    return Method(
      ...
      returnType: const TypeNormal(name: 'bool'),
      statements: IList([
...
class Method {
  ...
  String toCodeString() {
    return documentationCommentsToCodeString(documentationComments) +
        ...
        (typeParameters.isEmpty ? '' : '<' + stringListJoinWithComma(typeParameters) + '>') +
...
class Parameter {
  ...
  static String toCodeString(
    ...
    final namedCode = parameters.mapAndRemoveNull((parameter) => parameter._positionalArgumentToCodeString(isConstructor)).safeJoin();
    final positionalCodeList = parameters.mapAndRemoveNull(
      (parameter) => parameter._namedArgumentToCodeString(isConstructor),
    );
    return '(' + namedCode + (positionalCodeList.isEmpty ? '' : '{' + positionalCodeList.safeJoin() + '}') + ')';
  }
  ...
  String? _namedArgumentToCodeString(bool isConstructor) {
    return parameterPattern.match(
      ...
      namedWithDefault: (n) => _nameWithThis(isConstructor) + (n.constDefaultExpr is ExprNull ? '' : '= ' + n.constDefaultExpr.toCodeAndIsConst().toCodeString(true)) + ',',
    );
  }
...
class TypeFunction implements Type {
  const TypeFunction({required this.returnType, required this.parameterTypes, this.isNullable = false});
  final Type returnType;
  ...
  @override
  String toCodeString() {
    return returnType.toCodeString() + ' Function(' + parameterTypes.map((parameter) => parameter.toCodeString()).safeJoin(',') + ')' + (isNullable ? '?' : '');
  }
...
class TypeNormal implements Type {
  ...
  @override
  String toCodeString() {
    return name + (arguments.isEmpty ? '' : '<' + arguments.map((argument) => argument.toCodeString()).safeJoin(', ') + '>') + (isNullable ? '?' : '');
  }
...
class EnumDeclaration extends Declaration {
  ...
  String toCodeString() {
    return documentationCommentsToCodeString(documentationComments) +
        'enum ' +
        name +
        (implementsClassList.isEmpty ? '' : ' implements ' + implementsClassList.safeJoin(',')) +
        ' {\n' +
...
class EnumValue {
  ...
  String toCodeString() {
    return documentationCommentsToCodeString(documentationComments) + '  ' + name + ',\n';
  }
}
...
class StatementFinal implements Statement {
  ...
  String toCodeString() {
    ...
    return (codeAndIsConst.isConst() ? 'const' : 'final') + ' ' + variableName + ' = ' + codeAndIsConst.code + ';\n';
  }
}
...
class ExprCall implements Expr {
  ...
  CodeAndIsConst toCodeAndIsConst() {
    return CodeAndIsConst(
      (isAwait ? 'await ' : '') + functionName + _argumentsToString(positionalArguments, namedArguments).code,
      ConstType.noConst,
    );
  }
}
...
class ExprStringLiteral implements Expr {
  ...
  CodeAndIsConst toCodeAndIsConst() {
    ...
    final isNeedRaw = (value.contains(r'\') || value.contains(r'$')) && !value.contains('\n') && (!isDoubleQuote && !value.contains("'"));
    final escapedCommon = isNeedRaw ? value : value.replaceAll(r'\', r'\\').replaceAll(r'$', r'\$').replaceAll('\n', r'\n');

    return CodeAndIsConst(
      (isNeedRaw ? 'r' : '') + (isDoubleQuote ? '"' + escapedCommon + '"' : "'" + escapedCommon.replaceAll("'", r"\'") + "'"),
      ConstType.implicit,
    );
  }
}
...
class ExprMethodCall implements Expr {
  ...
  CodeAndIsConst toCodeAndIsConst() {
    return CodeAndIsConst(
      variable.toCodeAndIsConst().toCodeString(true) + (optionalChaining ? '?' : '') + '.' + methodName + _argumentsToString(positionalArguments, namedArguments).code,
      ConstType.noConst,
    );
  }
}
...
class ExprConstructor implements Expr {
  ...
  @override
  CodeAndIsConst toCodeAndIsConst() {
    final argumentsCodeAndIsConst = _argumentsToString(positionalArguments, namedArguments);
    return CodeAndIsConst(
      className + argumentsCodeAndIsConst.code,
      (isConst && argumentsCodeAndIsConst.isConst()) ? ConstType.explicit : ConstType.noConst,
    );
  }
}
  ...
  CodeAndIsConst toCodeAndIsConst() {
    ...
    return CodeAndIsConst(
      '{' +
          stringListJoinWithComma(
            IList(codeAndIsConstIter.map(
              (item) => item.first.toCodeString(!isAllConst) + ': ' + item.second.toCodeString(!isAllConst),
            )),
          ) +
          '}',
      isAllConst ? ConstType.explicit : ConstType.noConst,
...
class ExprIs implements Expr {
  ...
  CodeAndIsConst toCodeAndIsConst() {
    return CodeAndIsConst(
      '(' + expr.toCodeAndIsConst().toCodeString(true) + ' is ' + type.toCodeString() + ')',
      ConstType.noConst,
    );
  }
}
...
class ExprOperator implements Expr {
  ...
  CodeAndIsConst toCodeAndIsConst() {
    return CodeAndIsConst(
      '(' + left.toCodeAndIsConst().toCodeString(true) + operator.code + right.toCodeAndIsConst().toCodeString(true) + ')',
      ConstType.noConst,
    );
  }
}
...
class ExprConditionalOperator implements Expr {
  ...
  CodeAndIsConst toCodeAndIsConst() {
    return CodeAndIsConst(
      '(' + condition.toCodeAndIsConst().toCodeString(true) + ' ? ' + thenExpr.toCodeAndIsConst().toCodeString(true) + ' : ' + elseExpr.toCodeAndIsConst().toCodeString(true) + ')',
      ConstType.noConst,
    );
  }
}
...
CodeAndIsConst _argumentsToString(
  ...
  final positionalArgumentsCodeAndIsConst = positionalArguments.map((argument) => argument.toCodeAndIsConst());
  ...
  final isAllConst = positionalArgumentsCodeAndIsConst.every((argument) => argument.isConst()) && namedArgumentsCodeAndIsConst.every((argument) => argument.second.isConst());
  return CodeAndIsConst(
    '(' +
        stringListJoinWithComma(IList([
          ...positionalArgumentsCodeAndIsConst.map((argument) => argument.toCodeString(!isAllConst)),
          ...namedArgumentsCodeAndIsConst.map(
            (argument) => argument.first + ': ' + argument.second.toCodeString(!isAllConst),
          ),
        ])) +
        ')',
    isAllConst ? ConstType.explicit : ConstType.noConst,
